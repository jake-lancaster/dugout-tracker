...
const RESULTS = {
  ...
  GO: { label: "GO", color: "#fb923c",         bg: "#2d1a00",       wide: true  },
  FO: { label: "FO", color: "#fb923c",         bg: "#2d1a00",       wide: true  },
  K:  { label: "K",  color: "#f87171",         bg: "#3a1010",       wide: false },
  // legacy — old "O", "E", "W" records still render cleanly
  O:  { label: "O",  color: "#fb923c",         bg: "#2d1a00",       wide: false },
...
function SummarySubTab({ game, updateGame, players }) {
  ...
  return (
    <div>
      ...
      <div style={{ ...card, textAlign:"center" }}>
        ...
        <div style={{ marginTop:12, overflowX:"auto" }}>
          <table style={{ borderCollapse:"collapse", width:"100%", fontSize:12 }}>
            <thead>
              <tr>
                <th style={{ padding:"3px 6px", color:"var(--text-dim)", textAlign:"left", fontWeight:600 }}></th>
                {game.innings.map((_, i) => (
                  <th key={i} style={{ padding:"3px 6px", color:"var(--text-dim)", fontWeight:600, minWidth:28 }}>{i+1}</th>
                ))}
                <th style={{ padding:"3px 6px", color:"var(--text-dim)", fontWeight:700 }}>R</th>
              </tr>
            </thead>
            <tbody>
              {[["Us", totalUs, "var(--hit-text)"], ["Them", totalThem, "var(--out-text)"]].map(([lbl, tot, clr]) => (
                <tr key={lbl}>
                  <td style={{ padding:"3px 6px", fontWeight:600, textAlign:"left" }}>{lbl}</td>
                  {game.innings.map((inn, i) => (
                    <td key={i} style={{ padding:"3px 6px", color: (lbl==="Us" ? inn.runsUs : inn.runsThem) > 0 ? clr : "var(--text-dim)" }}>
                      {lbl === "Us" ? (inn.runsUs || 0) : (inn.runsThem || 0)}
                    </td>
                  ))}
                  <td style={{ padding:"3px 6px", fontWeight:800, color:clr }}>{tot}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Inning-by-inning batting */}
      <SectionLabel>BY INNING</SectionLabel>
      {game.innings.map((inn, i) => {
        const innABs = inn.atBats || [];
        if (innABs.length === 0) return null;
        // Sort at-bats by batting order position
        const sortedABs = [...innABs].sort((a, b) => {
          const ai = orderedIds.indexOf(a.playerId);
          const bi = orderedIds.indexOf(b.playerId);
          return (ai === -1 ? 999 : ai) - (bi === -1 ? 999 : bi);
        });
        const innHits = innABs.filter(ab => ab.result === "H").length;
        return (
          <div key={i} style={{ ...card }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
              <span style={{ fontSize:13, fontWeight:700, color:"var(--accent)" }}>Inning {i+1}</span>
              <span style={{ fontSize:12, color:"var(--text-dim)" }}>
                {inn.runsUs > 0 && <span style={{ color:"var(--hit-text)", fontWeight:700, marginRight:6 }}>{inn.runsUs} run{inn.runsUs !== 1 ? "s" : ""}</span>}
                {innHits}H · {innABs.length} AB
              </span>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
              {sortedABs.map((ab, j) => {
                const p = players.find(pl => pl.id === ab.playerId);
                const orderPos = orderedIds.indexOf(ab.playerId);
                return (
                  <div key={j} style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <span style={{ fontSize:11, color:"var(--text-dim)", minWidth:14, textAlign:"right" }}>
      ...
      {/* Season batting order totals */}
      <SectionLabel style={{ marginTop:8 }}>GAME TOTALS</SectionLabel>
      ...
      {orderedPlayers.map((p, i) => {
        ...
        return (
          <div key={p.id} style={{ ...card, padding:"8px 12px" }}>
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              <span style={{ color:"var(--accent)", fontSize:13, fontWeight:800, minWidth:20 }}>{i+1}</span>
              <span style={{ fontSize:13, fontWeight:600, flex:1 }}>#{p.number} {p.name}</span>
              <div style={{ display:"flex", gap:3 }}>
                {abs.map((ab, j) => <ResultBadge key={j} result={ab.result} />)}
              </div>
              <span style={{ fontSize:12, color:"var(--text-dim)", minWidth:32, textAlign:"right" }}>
                {stats.h}-{stats.ab}
              </span>
            </div>
          </div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:14 }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:6, marginTop:2 }}>
            ...
            {game.finished && <span style={{ fontSize:10, fontWeight:700, color:"var(--hit-text)",
              background:"var(--hit-bg)", borderRadius:4, padding:"1px 6px" }}>FINAL</span>}
          </div>
        </div>
        {!game.finished
          ? <button onClick={() => { updateGame(g => ({ ...g, finished: true })); onBack(); }}
              style={{ ...btnBase, background:"var(--hit-bg)", color:"var(--hit-text)",
                padding:"6px 12px", fontSize:12, fontWeight:700, border:"1px solid var(--hit-text)",
                whiteSpace:"nowrap" }}>
              End Game ✓
            </button>
          : <button onClick={() => updateGame(g => ({ ...g, finished: false }))}
              style={{ ...btnBase, background:"var(--tag-bg)", color:"var(--text-dim)",
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                  <span style={{ fontWeight:700, fontSize:15 }}>vs {g.opponent || "TBD"}</span>
                  {g.finished
                    ? <span style={{ fontSize:10, fontWeight:700, color:"var(--hit-text)",
                        background:"var(--hit-bg)", borderRadius:4, padding:"1px 6px" }}>FINAL</span>
                    : <span style={{ fontSize:10, fontWeight:700, color:"var(--accent)",
                        background:"var(--accent-dim)", borderRadius:4, padding:"1px 6px" }}>IN PROGRESS</span>
                  }
                </div>
